...
def test_slot_return_matches_payout_table(tmp_path):
    ...
    assert 0.95 <= want <= 1.10, f"отдача {want:.3f} — целимся около единицы"
        ...
        for c in range(n):
            for d in range(n):
                if a == c == d:
                    ...
                    ev -= bet
    ev /= n ** 3
    assert -0.1 <= ev <= 0.4, f"матожидание {ev:+.3f} балла за спин при ставке {bet}"

    # Не только СКОЛЬКО отдаётся, но и ГДЕ это лежит: если вся отдача в тройке
    # (1/36), счёт разоряется даже при честной единице — так и вышло на живом
    # прогоне, 79 спинов без единой тройки. Требуем, чтобы частые пары давали
    # заметную долю возврата, а не символическую.
    pair_share = b.SLOT_PAY_PAIR * 3 * (n - 1) / (n * n) / want
    assert pair_share > 0.6, (f"на пары приходится лишь {pair_share:.0%} отдачи — "
                              f"игра снова превращается в лотерею на джекпот")
