...
def main() -> None:
    ...
    # MJC_PARENT_SESSION — sessionId оркестратора, который завёл этого агента: ключ
    # связи «агент → команда». MJC_PARENT — его имя, оставлено справочно: имя не
    # уникально (видели три записи реестра с именем «слитие мастера»), меняется
    # /rename и переиспользуется после смерти сессии, поэтому ключом быть не может.
    # Обе переменные знает только плагин в момент спавна вкладки; в реестре Claude
    # Code таких полей нет. Родителя не удалось определить однозначно → плагин НЕ
    # ставит MJC_PARENT_SESSION вовсе, и это значит «связь неизвестна» (см. teams()).
    # Лимит у sessionId свой: имена режутся до 64, а UUID резать нельзя.
    for key, env, cap in (("agent", "CLAUDE_CODE_AGENT", 64),
                          ("sess_name", "CLAUDE_CODE_SESSION_NAME", 64),
                          ("parent", "MJC_PARENT", 64),
                          ("parent_sid", "MJC_PARENT_SESSION", 128)):
